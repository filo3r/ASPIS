...
/*
  ...
  commit c6a0d73e47bbd2bc86e34637156fb26dd4d5cf08,
  bench/sequential/statemate/statemate.c

  What the benchmark does: code generated from a statechart describing a car
  window lift controller. It holds around a hundred file-scope state
  variables, read and written by several functions, long chains of nested
  conditions and switches, and a 64-entry array holding the active states,
  accessed through the SYS_bit_* macros. The state machine is run for 100
  steps and the final check is a checksum over that array. That checksum
  shifts by up to 63 bits, so it assumes a 64-bit unsigned long.
