...
/*
  Integration notes
  -----------------
  Taken from https://github.com/tacle/tacle-bench
  commit c6a0d73e47bbd2bc86e34637156fb26dd4d5cf08, bench/sequential/ndes/ndes.c

  What the benchmark does: a DES encryption, built out of shifts, masks and
  table lookups on 64-bit integers, small structs passed and returned by
  value, and static local tables.

  Changes with respect to the TACLeBench version:
  - Added the functions DataCorruption_Handler and SigMismatch_Handler.
  - Added __attribute__((annotate("to_harden"))) to every file-scope variable
    and every function except main and the two handlers.
  - main prints SUCCESS or FAIL on stdout and always returns 0.
  - The result check is stronger: the original ndes_return only looks at the
    key-schedule state ndes_icd; it now also checks the encryption output
    ndes_out against the value produced by the unmodified benchmark.
  - ndes_des initialises ic to 0 before the first call to
    ndes_cyfun( itmp.l, kns[ ii ], &ic ). The callee writes *iout before
    reading it, so the result is unchanged.
  - Known upstream quirk left unchanged: in ndes_cyfun the last for loop has
    no braces, so the final OR into *iout runs once after the loop instead of
    once per iteration. The result is still deterministic.
*/
               ...
               ndes_immense *out )
{
  ...
  itmp.r = itmp.l = 0L;
  ic = 0L; /* not in the original benchmark: see the header */
...
int main( void )
{
  ...
  if ( ndes_return() == 0 && ndes_out.l == 19ul && ndes_out.r == 37ul )
    printf( "SUCCESS" );
  else
    printf( "FAIL" );
